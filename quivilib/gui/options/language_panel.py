...
class LanguagePanel(wx.Panel):
    def __init__(self, parent, available_languages: list[wx.Language], active_language: wx.Language):
        super().__init__(parent=parent, id=-1)

        self.lang_lst = langlistctrl.LanguageListCtrl(self, -1, style=wx.LC_REPORT | wx.LC_NO_HEADER, filter=langlistctrl.LC_ONLY, only=available_languages, select=active_language)
