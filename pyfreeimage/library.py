...
from pyfreeimage.funct_list import FUNCTION_LIST, FreeImageLibrary
...
def load(file = None) -> FreeImageLibrary:
    global lib
    if not lib:
        lib = Library(file)
    return lib
...
class Library(FreeImageLibrary):
    ...
    def get_fif_description(self, fif) -> str:
        return self.GetFIFDescription(fif).decode('latin-1')
